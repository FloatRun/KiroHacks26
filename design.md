...
| Property | Value |
|---|---|
| Data source | S3 bucket containing curated medical corpus |
...
- **Volume:** 50–150 documents
- **Format:** Plain `.txt` or `.md` files
- **Naming convention:** `{scenario-tag}-{source}-{sequence}.txt` (e.g., `burns-nhs-001.txt`)
...
│                                                         │
│  ┌──────────────┐    ┌──────────────────────────────┐  │
│  │  S3 (Corpus) │    │  SSM Parameter Store         │  │
│  │  Medical docs│    │  /firstaid-ai/places-api-key │  │
│  └──────────────┘    └──────────────────────────────┘  │
└─────────────────────────────────────────────────────────┘
                              │
                              ▼ (outbound HTTPS)
...
| Similarity threshold (0.5) | Empirically determined against the specific corpus. May need adjustment after ingestion. |
| Corpus document selection | Requires human judgment on source authority, coverage gaps, and scenario tagging. |
